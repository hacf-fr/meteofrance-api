...
def test_rain_other_http_error(requests_mock: Mock) -> None:
    """Test datecomputation when rain is expected within the hour."""
    client = MeteoFranceClient()

    requests_mock.request("get", f"{METEOFRANCE_API_URL}/rain", status_code=404)

    with pytest.raises(requests.HTTPError, match=r"404 .*"):
        client.get_rain(latitude=45.508, longitude=-73.58)


def test_rain_non_http_error(requests_mock: Mock) -> None:
    """Test datecomputation when rain is expected within the hour."""
    client = MeteoFranceClient()

    requests_mock.request(
        "get", f"{METEOFRANCE_API_URL}/rain", exc=requests.exceptions.InvalidURL
    )
